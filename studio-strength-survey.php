<?php
// LEGACY — superseded by /rssi.php (the RSSI app's own standalone landing).
...
if (!$uid) {
  ...
  header('Location: /login.html?return=' . urlencode('/studio-strength-survey.php' . $qs));
  exit;
}
$user = current_user();
...
$shell_body_attrs    = 'data-current-studio="strength-survey" data-studio-landing="strength-survey"';
...
<style>
  ...
  .studio-tile-card { display: flex; flex-direction: column; gap: 8px; padding: 22px 22px 20px; background: #fff; border: 1px solid var(--line); border-radius: 16px; text-decoration: none; color: inherit; transition: border-color 0.15s var(--ease), transform 0.15s var(--ease), box-shadow 0.15s var(--ease); min-height: 150px; }
  .studio-tile-card:hover { border-color: var(--landing-accent); transform: translateY(-2px); box-shadow: 0 8px 24px rgba(20, 28, 48, 0.08); }
  ...
  .studio-tile-card .tile-title { font-family: 'Fraunces', 'Georgia', serif; font-size: 19px; font-weight: 600; line-height: 1.25; color: var(--ink-1, #1c2238); }
  .studio-tile-card .tile-desc { font-size: 13.5px; line-height: 1.5; color: var(--ink-4, #5a657a); }
...
<section>
  <div class="studio-tiles">
    <a class="studio-tile-card is-primary" href="/studio-strength-survey-projects.php">
      ...
      <div class="tile-title">Pick a survey</div>
      <div class="tile-desc">Open one of your surveys and jump straight to its Overview.</div>
    </a>
    <a class="studio-tile-card" href="/strength-survey-wizard.php?step=1">
      <span class="icon"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg></span>
      <div class="tile-title">Upload data</div>
      <div class="tile-desc">Open the Survey Evidence Intake wizard. CSV, Excel, or a survey from scratch.</div>
    </a>
    <a class="studio-tile-card" href="/studio-strength-survey-projects.php?demo=1">
...
<section class="studio-section" aria-labelledby="recent-title">
  <div class="studio-section-head">
    ...
    <a class="head-meta" href="/studio-strength-survey-projects.php" style="text-decoration:none;color:var(--landing-accent);font-weight:600;">See all →</a>
  </div>
  ...
  (function () {
    ...
    fetch('/api/surveys/list.php', { credentials: 'same-origin', headers: { 'Accept': 'application/json' } })
      ...
      .then(function (data) {
        ...
        if (!list.length) {
          host.innerHTML = '<p class="recent-empty">No surveys yet. Click <strong>Upload data</strong> above to start one.</p>';
          return;
        }
        host.innerHTML = list.map(function (s) {
          const status = s.is_published ? 'Published' : 'Draft';
          return '<a class="recent-card" href="/studio-strength-survey.php?project_id=' + encodeURIComponent(s.id) + '">' +
