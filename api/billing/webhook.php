...
function apply_subscription_to_user(int $userId, array $sub): void
{
    ...
    // Update subscription row. The table has TWO unique keys (PRIMARY on
    // user_id, UNIQUE on stripe_subscription_id), so an INSERT can collide on
    // either. Include stripe_subscription_id in the UPDATE clause so a plan
    // change (cancel sub A, start sub B for the same user) overwrites the old
    // subscription id rather than leaving stale data.
    db()->prepare(
        'INSERT INTO subscriptions (user_id, stripe_subscription_id, status, tier, cycle, current_period_end, cancel_at_period_end)
         ...
         ON DUPLICATE KEY UPDATE stripe_subscription_id = VALUES(stripe_subscription_id),
                                  status = VALUES(status), tier = VALUES(tier), cycle = VALUES(cycle),
