...
// Gate paid checkout on a verified email. Prevents Stripe customers from
// being created with an unowned email and gives a clean billing trail.
try {
    ...
    if ($vrow && $vrow['email_verified_at'] === null) {
        fail('email_not_verified',
             'Please verify your email before subscribing. Check your inbox for the verification link, or request a new one from Account settings.',
             403);
    }
} catch (Throwable $e) {
    // Column may be missing pre-Phase 166. Skip the gate rather than break.
    error_log('[relicheck] checkout email-verify gate skipped: ' . $e->getMessage());
}
