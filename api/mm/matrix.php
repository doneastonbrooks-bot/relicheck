<?php
// GET  /api/mm/matrix.php?project_id=N
// POST /api/mm/matrix.php   { project_id }
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    $stmt = $pdo->prepare('SELECT * FROM mm_evidence_matrices WHERE project_id = :p ORDER BY position ASC, id ASC');
    $stmt->execute([':p' => $projectId]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    $out = [];
    foreach ($rows as $r) {
        $out[] = [
            'id' => (int)$r['id'], 'measure' => (string)$r['measure'],
...
$catStmt = $pdo->prepare(
    'SELECT c.name, c.description, (SELECT COUNT(*) FROM mm_coded_responses cr WHERE cr.category_id = c.id) AS n
...
$sentDist = [];
foreach ($sentStmt->fetchAll(PDO::FETCH_ASSOC) as $s) $sentDist[(string)$s['sentiment']] = (int)$s['n'];

if (count($cats) === 0 && count($aligns) === 0) fail('mm_no_inputs', 'Run the builder or alignment check before generating a matrix.');
...
$system = <<<SYS
You are a mixed-methods analyst. Compose a joint display table combining numbers, qualitative evidence, sentiment, interpretation, and a recommended action. 3-6 rows.

Return JSON only:

{
  "rows": [
    { "measure": "<label>", "quant_result": "<short>", "theme_evidence": "<short>", "sentiment": "<short>", "interpretation": "<one sentence>", "recommended_action": "<one sentence>" }
  ],
...
try {
    ...
    $ins = $pdo->prepare(
        'INSERT INTO mm_evidence_matrices (project_id, measure, quant_result, theme_evidence, sentiment, interpretation, recommended_action, position)
    ...
    foreach ($parsed['rows'] as $i => $r) {
        ...
        $ins->execute([
            ':p' => $projectId, ':m' => $row['measure'], ':q' => $row['quant_result'],
            ':te' => $row['theme_evidence'], ':s' => $row['sentiment'],
            ':i' => $row['interpretation'], ':ra' => $row['recommended_action'], ':pos' => $i + 1,
        ]);
        $rowsOut[] = $row;
    }
    $pdo->commit();
...
json_out(['ok' => true, 'rows' => $rowsOut, 'summary' => clean_string((string)($parsed['summary'] ?? ''), 600), 'model' => ai_config()['model']]);
