...
// Drops in a 30-row sample MM Studio project for testing.
...
try {
    ...
    $insSrc = $pdo->prepare(
        'INSERT INTO mm_data_sources (project_id, source_type, format, source_ref, label, row_count)
         VALUES (:p, "paste", "paste", NULL, :l, :rc)'
    );
    $insSrc->execute([':p' => $projectId, ':l' => 'Sample data', ':rc' => count($samples)]);
