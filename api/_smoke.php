<?php
// GET /api/_smoke.php?key=<email_cron_key>
//
// Phase 134b QA harness (minimal v2). Confirms the canonical schema tables
// exist in dbs15641829, and lints every PHP file in /api/ for the four
// known-bug patterns. Refactored from v1 to drop the RecursiveIterator and
// token_get_all calls that were OOMing the IONOS shared-hosting limits.
// We now glob a flat one-level-deep list, batch the work, and skip files
// we cannot read instead of fataling.
//
// Returns JSON. Add ?pretty=1 for human-readable output.

declare(strict_types=1);

// Diagnostic wrapper. Both halves removed together when done.
...
// Collect PHP files via glob (two levels deep covers the entire ReliCheck
// API tree without recursing into VCS metadata or vendor dirs).
$files = [];
...
foreach ($files as $path) {
    ...
    // Stray `?>` in `//` comments. Line-scan.
    $lines = explode("\n", $src);
    foreach ($lines as $i => $line) {
        $commentPos = strpos($line, '//');
        if ($commentPos === false) continue;
        $before = substr($line, 0, $commentPos);
        // Quote-balance heuristic so `//` inside a string literal does not trip.
