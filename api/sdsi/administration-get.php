<?php
// GET /api/sdsi/administration-get.php?survey_id=N&component=KEY
// Returns the saved (settled) administration review for a (survey, component)
// pair. Returns: { ok, exists, review? } where review = { component, context,
//           flags, mitigations, score, sdsiPoints, band, launchReady,
//           blockerCount, updatedAt }
...
if (!administration_component_spec($component)) fail('bad_component', 'Unknown administration component.');
...
$stmt = $pdo->prepare(
    'SELECT context, flags, mitigations, score, sdsi_points, band, launch_ready, blocker_count, updated_at
       FROM sdsi_administration_reviews WHERE survey_id = :sid AND component = :comp'
);
