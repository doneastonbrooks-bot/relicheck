<?php
// POST /api/sdsi/reliability-propose.php
// Body: {
//   survey_id: int,
//   component: string (one of the five factory reliability components),
//   context:   { ...reviewer-declared fields for this component... }
// }
//
// Reliability Readiness (factory lenses) — AI PROPOSER (not scorer).
// Reads the survey's item text server-side, asks the model to propose
// reliability-readiness issues against the component's check vocabulary,
// validates the shape, and returns a proposal the client seeds into the
// human-settled reviewer workflow. The deterministic factory lens (client)
// computes the score from the settled flags — this endpoint never asserts a
// number. ALPHA FENCE: this is a PRE-DATA design review; the prompt forbids
// alpha/omega/item-total/inter-item/factor analysis.
//
...
check_rate_limit('reliability_propose:user:' . (int)$user['id'], 60, 3600);
...
$spec = reliability_component_spec($component);
if (!$spec) fail('bad_component', 'Unknown reliability component.');
...
$system = reliability_system_prompt($spec);
...
$lines[] = '';
$lines[] = 'Review these items now. Use item_ref values that match the ids in brackets, a scale/subscale name, or a context field key (e.g. "score_reporting", "declared_scales") when the issue is in the declared context. Remember: PRE-DATA design review only — no alpha, omega, item-total, inter-item, or factor analysis.';
$userMsg = implode("\n", $lines);
...
// Authoritative required-item map (server-derived; never trusts AI). Stamped on
// each flag for spine parity with the validity proposer.
$requiredMap = [];
