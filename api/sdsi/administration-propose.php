<?php
// POST /api/sdsi/administration-propose.php
// Body: {
//   survey_id: int,
//   component: string (one of the five factory administration components),
//   context:   { ...reviewer-declared fields for this component... }
// }
//
// Administration Readiness (factory lenses) — AI PROPOSER (not scorer).
// Reads the survey's item text server-side, asks the model to propose
// administration-readiness issues against the component's check vocabulary,
...
// number. PRE-LAUNCH SCOPE: this is a readiness review, never a review of survey
// results or post-administration data quality (that belongs to RSSI).
//
...
check_rate_limit('administration_propose:user:' . (int)$user['id'], 60, 3600);
...
$system = administration_system_prompt($spec);
...
$lines[] = '';
$lines[] = 'Review these items now. Use item_ref values that match the ids in brackets, a section name, or a context field key (e.g. "purpose", "participation_status") when the issue is in the declared context. Remember: PRE-LAUNCH readiness review only — never evaluate survey results or post-administration data quality.';
$userMsg = implode("\n", $lines);
...
// each flag for spine parity with the validity + reliability proposers.
$requiredMap = [];
