...
//                    /evidence-intake.php?studio=survey&wizard=1)
...
//
// On complete: redirect to /strength-index.php?studio=survey&project_id=N
...
if (!$uid) {
  ...
  header('Location: /login.html?return=' . urlencode('/survey-wizard.php' . $qs));
  exit;
}
$user = current_user();
...
if ($projectId > 0) {
  ...
  if (!$project) { header('Location: /survey-wizard.php?step=1'); exit; }
}
if ($step > 1 && !$project) { header('Location: /survey-wizard.php?step=1'); exit; }
...
$shell_body_attrs    = 'data-current-studio="survey" data-studio-landing="survey" data-survey-wizard="' . $step . '"';
...
<div class="wiz-shell">
  ...
  <div class="wiz-top">
    <a class="wiz-back" href="/studio-survey.php">
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
      Back to Survey Studio
    </a>
    <a class="wiz-back" href="/studio-survey-projects.php" style="color:var(--ink-5);">All surveys &rarr;</a>
  </div>
...
<?php if ($step === 1): ?>
    ...
    <form id="svWizStep1">
      ...
      <div class="wiz-actions">
        <a class="btn-ghost" href="/studio-survey.php">Cancel</a>
...
<?php elseif ($step === 2): ?>
    ...
    <div class="wiz-intake-box">
      ...
      <a href="/evidence-intake.php?studio=survey&amp;project_id=<?= (int)$projectId ?>&amp;wizard=1">Open Evidence Intake →</a>
    </div>
    <div class="wiz-actions">
      <a class="btn-ghost" href="/survey-wizard.php?step=1&amp;project_id=<?= (int)$projectId ?>">&larr; Back</a>
      <a class="btn-primary" href="/survey-wizard.php?step=3&amp;project_id=<?= (int)$projectId ?>" style="text-decoration:none;">Skip to scales step &rarr;</a>
    </div>
...
<?php elseif ($step === 3): ?>
    ...
    <div class="wiz-actions">
      <a class="btn-ghost" href="/survey-wizard.php?step=2&amp;project_id=<?= (int)$projectId ?>">&larr; Back</a>
...
(function () {
  ...
  if (step1Form) {
    step1Form.addEventListener('submit', function (e) {
      ...
      fetch(url, {
        ...
        .then(id => {
          window.location.href = '/survey-wizard.php?step=2&project_id=' + encodeURIComponent(id);
        })
        .catch(e => {
  ...
  if (scaleHost) {
    ...
    if (!dataset || !dataset.variables) {
      ...
      if (!likertItems.length) {
        ...
        html += '</tbody></table>' +
          ...
          // spec §4E sub-component 2 tri-state). Distinguishes "user has
          // reviewed every item for reverse-coding and confirms the
          // present flags are complete" from "user hasn't completed
          // reverse-coding flagging yet." Without this checkbox, §4E
          // sub-2 skip-and-rescales rather than scoring zero on the
          // architectural-finding case.
    ...
    document.getElementById('svWizFinishBtn').addEventListener('click', function () {
      ...
      // Survey-level reverse-coding confirmation (KNOWN_ISSUES.md §4 #3).
      // Stored alongside the per-row scale map in localStorage under a
      // sentinel key (preserves the {[varname]: row} shape for the
      // per-row loader above); sent to the surveys.settings JSON as a
      // sibling of scales.
      ...
      // Bundling scales + reverse_coded_confirmed in one PATCH because
      // api/surveys/update.php replaces the entire settings object on
      // each call; sending only `scales` would wipe the confirmation
      // (and vice versa).
      fetch('/api/surveys/update.php', {
        ...
        .catch(() => ({ ok: false }))
        .finally(() => {
          window.location.href = '/strength-index.php?studio=survey&project_id=' + encodeURIComponent(PROJECT_ID);
        });
    });
  }
