...
const jd={base:null,busy:false,err:'',picking:false,choosing:null,candidates:null,cbusy:false};
...
function jdChoose(themeId){if(jd.choosing===themeId){jd.choosing=null;jd.candidates=null;renderJoint(activeStep());return;}jd.choosing=themeId;jd.candidates=null;jd.cbusy=true;renderJoint(activeStep());jdEvidenceFetch(themeId).then(j=>{jd.cbusy=false;jd.candidates=(j&&j.ok)?(j.responses||[]):null;renderJoint(activeStep());}).catch(()=>{jd.cbusy=false;jd.candidates=null;renderJoint(activeStep());});}
...
function jdCandidatePanel(){if(jd.choosing==null)return '';const t=((jd.base&&jd.base.rows)||[]).find(r=>r.theme_id===jd.choosing);const name=t?t.theme_name:'';if(jd.cbusy)return `<div class="th-quotes" style="padding:16px 18px">Loading responses…</div>`;const c=jd.candidates;if(!c||!c.length)return `<div class="th-quotes" style="padding:16px 18px">No coded responses to choose from for ${esc(name)}. Tag responses on the Qualitative Themes step first.</div>`;const rows=c.slice(0,15).map(r=>`<div class="th-quote"><div class="th-quote-t">“${esc(r.text)}”</div><div class="th-quote-m">${r.respondent_ref?'<span>'+esc(r.respondent_ref)+'</span>':''}${r.sentiment?'<span>· '+esc(r.sentiment)+'</span>':''} <button class="btn" style="padding:2px 9px" onclick="jdSetQuote(${jd.choosing},${r.response_id})">Use this quote</button></div></div>`).join('');return `<div class="ov-sec" style="margin-top:2px">Choose a quote for ${esc(name)}</div><div class="th-quotes">${rows}</div>`;}
