...
const th={base:null,busy:false,err:'',building:false,coding:false,codingAi:false,sel:null,quotes:null,qbusy:false};
...
function renderThemes(s){
  ...
  const busy=th.coding||th.codingAi;
  const codeBar=`<div class="dm-save"><button class="btn primary" ${busy?'disabled':''} onclick="thCodeAi()">${th.codingAi?'Tagging with ReliCheck Intelligence…':(allZero?'Tag responses with ReliCheck Intelligence':'Re-tag with ReliCheck Intelligence')}</button><button class="btn" ${busy?'disabled':''} onclick="thCode()">${th.coding?'Tagging…':'Keyword tag'}</button><span class="dm-note">${allZero?'Your themes are not tagged to any responses yet. ReliCheck Intelligence reads meaning (recommended); keyword tagging is a fast literal-match pass.':'ReliCheck Intelligence reads meaning, so it catches themes worded differently. Keyword tagging is a fast literal match.'}</span></div>`;
