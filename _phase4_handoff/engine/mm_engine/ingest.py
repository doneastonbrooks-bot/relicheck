...
def _finalize(df: "pd.DataFrame", path: str, fmt: str) -> dict:
    ...
    # Phase 4: cache the full DataFrame so subsequent analysis calls can
    # read whole columns without round-tripping every row through the
    # webview.
    ingest_id = _dataset.put_dataset(df, fmt, path)
