/* ════════════════════════════════════════════════════════════════════════
   Reliability Readiness LOCKSTEP test
   ────────────────────────────────────────────────────────────────────────
   The client spec (apps/sdsi/reliability-specs.js) drives the engine + UI;
   the server mirror (api/sdsi/_reliability_prompts.php) is the validation gate
   for AI proposals and the prompt content. They MUST agree on the load-bearing
   vocabulary or the proposer/validator and the scorer drift apart.
...
var JS = require('./reliability-specs.js');
...
var phpFile = path.resolve(__dirname, '../../api/sdsi/_reliability_prompts.php');
var phpDump = '' +
  "require '" + phpFile + "';" +
  "$out = ['order' => reliability_components(), 'specs' => []];" +
  "foreach (reliability_components() as $k) {" +
  "  $s = reliability_component_spec($k);" +
  "  $cf = [];" +
