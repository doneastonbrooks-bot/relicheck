...
**Per-domain status with current data contract (as of 2026-05-27, after #1a + #1c landed).**

- **§4E Scale Structure**: **Survey Builder path ACTIVE** for surveys with constructs assigned (item-count, missingness, survey health score). Uploaded-datasets path still whole-domain-skips pending #1b. #4 unlocks sub-3 across all surfaces. #2 + #3 together unlock sub-2 across all surfaces.
...
- **§4A Validity**: **Survey Builder path ACTIVE** (convergent + HTMT score). Uploaded-datasets path still skips pending #1b. #6 unlocks criterion and disengages the §3.6 V-F cap.
- **§4B Construct Alignment**: **Survey Builder path ACTIVE** (per-scale PAF loadings + cross-loadings score). Uploaded-datasets path still skips pending #1b.
