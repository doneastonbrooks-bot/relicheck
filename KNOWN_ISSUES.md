...
All three survey-construction surfaces (Survey Builder, wizards-driven settings, uploaded XLSX/CSV datasets) now flow construct assignments through to the engine. The standalone RSSI app (§16) remains the largest activation gap.

- **§4E Scale Structure**: **ACTIVE across all three survey-construction surfaces** for items with constructs assigned (item-count, missingness, survey health score). Standalone RSSI (§16) still whole-domain-skips. #4 unlocks sub-3 across all surfaces. #2 + #3 together unlock sub-2 across all surfaces.
- **§4G Response Scale Review**: partial-evaluation. Per-scale straight-lining now active across all three survey-construction surfaces. #4 unlocks anchor count / midpoint / single-format. #5 unlocks real anchor-symmetry (currently default 2/2 per §8).
- **§4A Validity**: **ACTIVE across all three survey-construction surfaces** (convergent + HTMT score). Standalone RSSI (§16) still skips. #6 unlocks criterion and disengages the §3.6 V-F cap.
- **§4B Construct Alignment**: **ACTIVE across all three survey-construction surfaces** (per-scale PAF loadings + cross-loadings score). Standalone RSSI (§16) still skips.
