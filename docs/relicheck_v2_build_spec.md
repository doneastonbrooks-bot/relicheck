...
| Fairness (DIF proxy) | 8 | When demographic columns are configured: for each Likert item, compute the response-mean difference across the largest two groups in each demographic column. Flag items with |mean_diff| ≥ threshold as DIF candidates. **Threshold normalization**: when per-item `likert_range` or `anchor_count` metadata is available, threshold = 12.5 % of scale range (so a 5-pt scale uses 0.5; a 7-pt uses 0.75) so the flag rate is calibrated consistently across formats. When anchor metadata is absent, fall back to 0.5 absolute and surface a `scale_range_normalization_unavailable_using_absolute_threshold` diagnostic on the §4D output. **Minimum-N floor**: require N ≥ 30 in each of the two largest groups for an item to be eligible for DIF flagging; items where the second-largest group falls below 30 valid responses skip the check (neither flag nor credit). **Multiple demographic columns**: per Phase 1 Q4, flag the item if ANY configured demographic column shows mean-diff ≥ threshold; max 1 deduction per item; diagnostics surface every triggering column, not just the first. Start at 8, deduct 1 pt per flagged item, clamped to [0, 8]. Use this cheap demographic-slice proxy, not full Mantel-Haenszel, in v2 phase one. When no demographic columns are configured: skip this sub-component, rescale wording-health alone to 0–100 by multiplying by 100/12 ≈ 8.33, and surface a "no demographics provided" indicator. Do not zero-fill the fairness component. |

Per-flag deduction (Phase 1 Q9): the wording table awards 1 pt deduction *per flag*, not per item, so a single item tripping all three flags loses 3 pts of the 12 wording pool.

**Item-text "has real prompt text" check** (Phase 1 Q1). Wording health applies to Likert and open-ended items with real prompt text on `v.label`. Items where `label === name` (no propagation), label is empty, or label has fewer than 3 word-tokens are skipped from inspection. When *every* Likert/open item is text-less, the wording half whole-skips and only fairness scores (if demographics are configured).

**Demographic detection** (Phase 1 platform contract). The engine accepts either a per-variable `is_demographic: true` opt-in flag OR a `config.demographic_columns: [name, …]` list; whichever the platform supplies wins. Engine logic is `v.is_demographic === true || config.demographic_columns.includes(v.name)`.

**Skip-and-rescale** matches §4E / §4G / §4A. Whole-domain skip when neither item text nor demographic columns are available. Wording-only when demographics absent (rescale ×100/12 ≈ 8.33). Fairness-only when wording absent (rescale ×100/8 = 12.5) — symmetric per Phase 1 Q7.

Wording health is always score-able *when item-text is propagated*; the data-contract gap (when label === name on every item) triggers a skip rather than producing a misleading score. Fairness requires demographics; when absent, the domain still produces a score from wording alone but flags the absence visibly on the domain detail page.
