-- ReliCheck — SDSI Access Readiness review persistence.
-- Run once via phpMyAdmin. Idempotent (CREATE TABLE IF NOT EXISTS).
--
-- Access is the second PRE-DATA validity lens inside the Instrument Quality
-- app (the first is Dignity / Framing). The AI proposes access barriers; a
-- human settles each one; the deterministic AccessEngine scores from the
-- settled flags. This table stores one settled review per survey (the latest),
-- so the score, the evidence ledger, and the launch-gate state survive a reload
-- and feed the report.
--
